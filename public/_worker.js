export default {
  async fetch(request, env, ctx) {
    ...
    if (host === 'react-schoolplanner.pages.dev' || host.endsWith('.react-schoolplanner.pages.dev')) {
      // Serve a SW killer script if the old app tries to access /sw.js
      if (url.pathname === '/sw.js') {
        const body = `self.addEventListener('install', e => self.skipWaiting());self.addEventListener('activate', e => {e.waitUntil((async()=>{try{const regs=await self.registration.unregister();const names=await caches.keys();await Promise.all(names.map(n=>caches.delete(n)));const cs=await self.clients.matchAll({includeUncontrolled:true});cs.forEach(c=>c.navigate(c.url))}catch{}})())});`;
      ...
      return new Response(MOVED_PAGE_HTML, {
        ...
        headers: {
          ...
          // Prevent any caching by browsers or proxies
          'cache-control': 'no-store, no-cache, must-revalidate, max-age=0',
          'pragma': 'no-cache',
          'expires': '0',
          'surrogate-control': 'no-store',
          // Ask the browser to clear caches/cookies and terminate service workers for this origin
          'clear-site-data': '"cache", "cookies", "executionContexts"'
        }
      });
    }
